...
class EntityExtractor:
    ...
    def __init__(self, Lang='ko_core_news_sm'):
        # 생성자 함수: EntityExtractor 객체를 초기화
        # 매개변수 Lang: 사용할 spaCy 언어 모델명 (기본값: 한국어 모델)
        # 언어별로 다른 NLP 모델이 필요하며, 한국어 텍스트 처리를 위해 한국어 모델을 기본으로 설정
        ...
        try:
            ...
            # 예외 처리: 지정된 언어 모델이 설치되지 않은 경우
            # 한국어 모델이 없을 때 대안으로 영어 모델 사용하여 프로그램 중단 방지
            self.nlp = spacy.load('en_core_web_sm')
            # 영어 모델로 대체: 한국어 모델 로드 실패 시 영어 모델 사용
    ...
    def extract_entities(self, text):
        ...
        entities = []
        # 빈 리스트 초기화: 추출된 개체 정보를 저장할 리스트

        for ent in doc.ents:
            # 개체 순회: Doc 객체에서 인식된 모든 개체를 반복 처리
            # doc.ents: spaCy가 인식한 명명된 개체들의 컬렉션
